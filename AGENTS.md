...
- `POST /api/projects/{id}/messages/{messageID}/resolution` — human-session-only (API keys forbidden), idempotent resolve/dismiss record for a held action request; it never releases or rewrites the held message (PAI-905)
...
- `GET|POST /api/projects/{id}/baseline-batches…` — opt-in Aithema handover draft, owned `inspr.readiness.v1` probe, immutable baseline batch mapped onto delivery attempts, the typed built-receipt producer for implementation/QA (`POST .../built-receipt`; `paimos baseline-batch report-built`), and the PAI-960 bridge from reviewed specification through scoped implementation/QA evidence into existing Pharos external-stage handoffs (`POST .../reconcile`; see [`docs/api-minimal.md`](docs/api-minimal.md))
...
- `paimos-agentd serve --instance <ppm-instance>` — operator-local owner for managed Codex, Claude, Pi, and Cursor children; private per-instance Unix socket; Claude requires Node.js 18+, Claude CLI 2.1.251+, and an explicitly configured operator-installed Agent SDK 0.3.251; Pi requires `--pi-path` and `--pi-accounts` mapping opaque keys to protected `PI_CODING_AGENT_DIR` directories; Cursor requires pinned CLI `2026.09.02-c22c1a3`, official ACP stdio (`--cursor-path` optional when `cursor-agent` is on PATH), and `--cursor-accounts` mapping opaque keys to expected vendor identity (email/userId; tokens stay in the vendor store)
- `paimos tell <harness>:<agent> --project PAI --level simple|steer -m <text>` — durable message with receiver interruption intent; `--expects-reply` opens an explicit durable reply obligation and `--action-request` holds it for humans (PAI-826 / PAI-905)
- Owned Codex/Claude sessions have `paimos_send_message` / `mcp__paimos__send_message` for bounded native replies (PAI-1041). Use incoming `message_id` as `reply_to`; sender/project/session/proof come from the runtime. Requires matching server/CLI/daemon builds and a fresh owned session; see `docs/AGENT_INTERCOM.md`.
- `paimos message target set --project PAI --address <harness>:<agent> --adapter codex|agentd_codex|agentd_claude|agentd_pi|agentd_cursor|grok_bot_routine|claude_resume|claude_channel --kind codex_thread|agentd_session|https_webhook|claude_session …` — register an encrypted receiver target; `grok_bot_routine` additionally requires `--target-key-file <file|->` (the routine sender key, sent as `Authorization: Bearer` on every wake; never an argument); `message target list` / `message deliveries` show redacted state (PAI-826 / PAI-827 / PAI-828 / PAI-849 / PAI-850)
- `paimos listen --as <harness>:<agent> --project PAI [--follow] [--ack] [--deliver codex|agentd_codex|agentd_claude|agentd_pi|agentd_cursor|claude|grok]` — durable native inbox delivery using the receiver-owned target (`--deliver-target` only for pre-bus rows); Grok Build additionally requires `--enable-grok-build-delivery` (PAI-816 / PAI-826 / PAI-849 / PAI-850)
- `paimos harness register|list|status|orchestrator|bind|heartbeat|yield|drain|complete-delivery|interrupt|stop|complete-control|mark-stopped ...` — durable harness-session lifecycle and explicit binding CAS, with steer-named drain/completion compatibility aliases; distinct from attribution-only `paimos session start` (PAI-848 / PAI-903)
