...
- `paimos-agentd serve --instance <ppm-instance>` — operator-local owner for managed Codex, Claude, and Pi children; private per-instance Unix socket; Claude requires Node.js 18+, Claude CLI 2.1.251+, and an explicitly configured operator-installed Agent SDK 0.3.251; Pi requires `--pi-path` and `--pi-accounts` mapping opaque keys to protected `PI_CODING_AGENT_DIR` directories
