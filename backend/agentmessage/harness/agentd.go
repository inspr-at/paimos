...
const (
	...
	agentdCodexSteerPrimitive = "codex app-server turn/steer"
)
...
func (AgentdCodexPlugin) Deliver(ctx context.Context, request DeliverRequest) (DeliverResult, error) {
	return deliverAgentdSteer(ctx, request, agentdCodexSteerPrimitive, false)
}
...
func deliverAgentdSteer(ctx context.Context, request DeliverRequest, expectedPrimitive string, requireVendorMessage bool) (DeliverResult, error) {
	...
	if receipt.CorrelationID != request.CorrelationID || receipt.Primitive != expectedPrimitive ||
	...
	if requireVendorMessage && (receipt.VendorMessageID == "" || len(receipt.VendorMessageID) > 256 || strings.ContainsAny(receipt.VendorMessageID, "\x00\r\n")) {
		return DeliverResult{}, errors.New("agentd returned incomplete Claude control evidence")
	}
