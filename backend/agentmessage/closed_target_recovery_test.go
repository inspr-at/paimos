...
func insertRecoverySession(t *testing.T, project int64, targetID, phase, machine string) string {
	t.Helper()
	return insertRecoverySessionOwned(t, project, targetID, phase, machine, `{"account_label":"chatgpt"}`, "chatgpt", "", "", "", "", "")
}

func insertRecoverySessionOwned(t *testing.T, project int64, targetID, phase, machine, registration, account, key, profileID, profileVersion, profileModel, profileEffort string) string {
	t.Helper()
	var agent int64
	...
	if _, err := paimosdb.DB.Exec(`INSERT INTO lifecycle_runtimes(id,project_id,generation,machine_id,user_id,api_key_id,
		...
		runtimeID, project, runtimeGeneration, machine, userID, apiKeyID, registration); err != nil {
		t.Fatal(err)
	}
	if _, err := paimosdb.DB.Exec(`INSERT INTO harness_sessions(id,project_id,project_agent_id,agent_name,harness,host,
		...
		account_label,account_key,dispatch_profile_id,dispatch_profile_version,dispatch_model,dispatch_effort,
		workspace_identity,workspace_path,workspace_kind,workspace_mode,
		...
		strftime('%Y-%m-%dT%H:%M:%fZ','now'),?,?,?,?,?,?,printf('%064d',1),'/fixture','directory','exclusive',?,?,?,?)`,
		sessionID, project, agent, machine, targetID, phase, account, key, profileID, profileVersion, profileModel, profileEffort, activityState, activityReason, activityKind, closedReason); err != nil {
		t.Fatal(err)
	}
...
func TestClosedTargetRecoveryV3ScopeFenceMatchAndCrossClass(t *testing.T) {
	v3Match := `{"schema_version":3,"account_scopes":[{"account_label":"chatgpt","profiles":[{"id":"codex-sol-high","version":"1"}]},{"account_label":"cursor_context","accounts":[{"key":"cursor-op","label":"Cursor"}],"profiles":[{"id":"cursor-composer","version":"1"}]}]}`
	v3CursorOnly := `{"schema_version":3,"account_scopes":[{"account_label":"cursor_context","accounts":[{"key":"cursor-op","label":"Cursor"}],"profiles":[{"id":"cursor-composer","version":"1"}]}]}`
	t.Run("match", func(t *testing.T) {
		f := newClosedTargetFixtureOwned(t, v3Match, v3Match)
		plan, err := f.service.InspectClosedTargetRecovery(context.Background(), f.input)
		if err != nil {
			t.Fatalf("matching v3 recovery refused: %v", err)
		}
		f.input.ExpectedTargetID, f.input.ExpectedTargetVersion = plan.EffectiveTarget.TargetID, plan.EffectiveTarget.TargetVersion
		f.input.ExpectedConsumerFence = plan.ConsumerFence
		if _, err := f.service.RecoverClosedTarget(context.Background(), f.input); err != nil {
			t.Fatalf("matching v3 recovery apply refused: %v", err)
		}
	})
	t.Run("cross-class", func(t *testing.T) {
		f := newClosedTargetFixtureOwned(t, v3Match, v3CursorOnly)
		if _, err := f.service.InspectClosedTargetRecovery(context.Background(), f.input); err == nil {
			t.Fatal("cross-class v3 replacement accepted")
		}
	})
}

func newClosedTargetFixtureOwned(t *testing.T, oldRegistration, newRegistration string) closedTargetFixture {
	t.Helper()
	service, project := openBusTestDB(t)
	allowBusSender(t, service, project, "codex:amy")
	result, err := paimosdb.DB.Exec(`INSERT INTO users(username,password,role,role_key,status,is_super_admin) VALUES('recovery-admin','fixture','admin','super_admin','active',1)`)
	...
	oldSession := insertRecoverySessionOwned(t, project, oldTarget.ID, "stopped", "old-machine", oldRegistration, "chatgpt", "", "codex-sol-high", "1", "codex", "high")
	message, err := service.SendEnvelope(context.Background(), SendEnvelopeInput{
		ProjectID: project, Sender: "sender", To: "codex:amy", Body: "recoverable fixture", DeliveryLevel: "steer",
	})
	if err != nil {
	...
	newSession := insertRecoverySessionOwned(t, project, newTarget.ID, "working", "new-machine", newRegistration, "chatgpt", "", "codex-sol-high", "1", "codex", "high")
	deliveries, err := service.ListDeliveryStatus(context.Background(), project)
	if err != nil || len(deliveries) != 1 || deliveries[0].MessageID != message.MessageID {
		t.Fatalf("deliveries=%+v err=%v", deliveries, err)
	}
