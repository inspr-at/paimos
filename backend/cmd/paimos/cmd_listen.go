...
//
// Unmanaged Claude sessions have no steer primitive: there is no `claude
// steer`, no send-to-session CLI, and no documented messaging-socket user
// frame. These legacy handoffs remain simple-only. The separate agentd_claude
// plugin controls only a Paimos-owned live Agent SDK Query.
const (
