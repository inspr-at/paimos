...
func runSecretsRotate(args []string) {
	...
	if *dryRun {
		fmt.Printf("dry-run: would rotate %d CRM provider config row(s), %d AI settings row(s), and %d agent-message target row(s) (%d with a sender secret).\n",
			report.CRMRows, report.AIRows, report.AgentMessageTargetRows, report.AgentMessageTargetSecretRows)
		fmt.Println("(decrypts confirmed under the current PAIMOS_SECRET_KEY; nothing was written.)")
		return
	}

	fmt.Printf("✔ rotated %d CRM provider config row(s), %d AI settings row(s), and %d agent-message target row(s) (%d with a sender secret).\n",
		report.CRMRows, report.AIRows, report.AgentMessageTargetRows, report.AgentMessageTargetSecretRows)
	fmt.Println()
