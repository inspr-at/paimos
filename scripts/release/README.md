...
- the certificate is re-issuable at any time from
  [developer.apple.com](https://developer.apple.com/account/resources/certificates)
  ("Developer ID Application"), so a lost `.p12` is an inconvenience, not a
  loss — you re-issue, re-export, re-upload;
...
warns 120 days out and fails once expired (`Check signing certificate
expiry` in `release-v2.yml`), which is the renewal trigger — there is no
calendar entry to forget.
