...
| Durable agent handoffs · attributable project-scoped threads with simple/steer intent, encrypted receiver targets, and exact vendor-native delivery to Codex, Claude Code and Grok Bot receivers with explicit security boundaries | shipped | M151–M155 `agent_messages`, receiver cursors and allowlists, encrypted versioned `agent_message_targets`, and `agent_message_deliveries` outbox rows; `paimos tell --level`, `message target|deliveries`, and `listen --deliver`; issue-linked human visibility; typed action-request holds; untrusted-data framing; exact Codex `queue`/`turn/steer` delivery with deterministic simple fallback, Grok Bot `grok_bot_routine` HTTPS wake, and Claude Code channel push plus `claude -p --resume|--cloud` simple delivery (PAI-815/816/817/825/826/827; see `docs/AGENT_MESSAGE_SECURITY.md`, `docs/AGENT_BUS_ARCHITECTURE.md`, `docs/AGENT_INTERFACE.md`, and the live runtime proof in `docs/REFERENCE_DEPLOYMENTS.md`) | — |
