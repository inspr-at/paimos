...
| Durable agent handoffs · attributable project-scoped threads delivered to vendor-native Claude Code, Codex and opt-in Grok Build sessions with explicit security boundaries | shipped | M152/M153 `agent_messages`, receiver cursors and allowlists; `paimos tell`, `message` and `listen`; issue-linked human visibility; typed action-request holds; untrusted-data framing; and fixed vendor-native adapters (PAI-815/816/817; see `docs/AGENT_MESSAGE_SECURITY.md`, `docs/AGENT_INTERFACE.md`, and the live two-way proof in `docs/REFERENCE_DEPLOYMENTS.md`) | — |
