...
| Agent Intercom · attributable project-scoped threads with durable simple/steer intent, encrypted receiver targets, truthful fallback, and owned Codex/Claude status, steer, interrupt, and stop | shipped | M151–M168 message, delivery, target, harness-session, and generation-worker-lease contracts; `paimos tell --level`, `message target|deliveries`, `listen --deliver`, and `paimos-agentd`; exact owned Codex app-server Process and Claude Agent SDK Query controls; per-generation worker authorization with digest-only server storage; restart reconciliation to `ownership_lost`; unmanaged Claude and Grok remain simple-only (PAI-815/816/817/825/826/827/828/829/848/849/850/870; see `docs/AGENT_INTERCOM.md`, `docs/AGENT_MESSAGE_SECURITY.md`, `docs/AGENT_INTERFACE.md`, and focused `agentmessage`, `managedharness`, and `agentd` tests) | — |
