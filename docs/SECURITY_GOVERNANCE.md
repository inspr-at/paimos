...
|---|---|---|---|
| **Days since last release** | newest supported release tag (`source scripts/release-version.sh; git tag --sort=-creatordate \| release_version::tag_filter \| head -1`) + date | < 60 d (no commitment) | a project that hasn't released in a while is either stable, on hiatus, or abandoned — readers can read the signal honestly |
