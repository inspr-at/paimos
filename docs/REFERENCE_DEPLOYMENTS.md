...
The register contains two validated deployment histories. As of 2026-06-30, ppm is the only active deploy target; the second-operator instance is historical and inactive since the operator exit in June 2026. The current ppm proof is the live `26.09.07` server deployment verified on 2026-09-07 with public `/api/health` `200/ok`, deployment identity `ppm`, exact source `1652e1a3fa04207aed37eb4eea0bb236746463fa`, and image `ghcr.io/inspr-at/paimos:26.09.07@sha256:60264e059c66cc8abaf9eb0803635cb7f58e94ec1b3dbef5b14cbf2d4db275f4`. Full backend CI run `34094550889` passed against the exact source; image signature, two SBOM attestations, provenance, and the claim gate passed before nixcfg PR 540 merge `289791bbe5d19e0ee4276fa926d98535fa9d7efb`. A fresh stopped-volume backup was captured at `/home/mba/paimos-backups/ppm/20260907T084321Z/csb1_ppm_data.tar.gz` with `manifest.json`; the archive SHA-256 is `09266ad1a85233b771721d33610fd2c248cfe2434d6de9f826e1ad218c1e62fe`. The matching Mac CLI/daemon report `26.09.07`; post-live verification passed 12 installed-CLI black-box checks and all 6 headless-browser fixtures against static assets extracted from the actual running container. `index-CAT5wJ9i.js` SHA-256 `38d0d1b10b8fac36c9a61af467795b841406c7a7c2b35e478aa4d13a6c320051` matched the exact container asset. Runtime doctor retains the baseline `targets ownership unverified`, `receiver not configured`, and `primary inbox unavailable` layers; human-session runtime-health, clean-OS onboarding, named cross-machine handoffs, and VoiceOver/accessibility remain open. The UI draft-refresh follow-up is tracked in [PAI-950](https://pm.barta.cm/projects/6/issues/PAI-950). The public marketing site remains on prior `26.09.06.21.31` provenance, so this record makes no new public-site claim. Operational evidence: [PAI-947](https://pm.barta.cm/projects/6/issues/PAI-947) + [PAI-949](https://pm.barta.cm/projects/6/issues/PAI-949).

The preceding `26.09.06.21.31` verification remains historical: it covered the live server, exact image/source identity, matching Mac CLI/daemon and lifecycle mapping, release-bound public site, and scoped live UI evidence. Operational evidence was PAI-928; `04-exact-live-verification.json`, SHA-256 `b0c2b36946a3260a3292103377132e31a87ce8ee80522d53a4168927983bc89e`; `mac-2131-activation-receipt.json`, SHA-256 `f320ed979c798be5632102404569d0fe52d57b1b992cc67711f7d1936c16fbcc`; post-install `receipt.json`, SHA-256 `1728eca8df1180b35ee266e7bf96e7d35a5eeb957e8402da27c28fb3ea235819`; site `receipt.json`, SHA-256 `2edbc16fd8f6a38876f4d5eb0944d3dbfc08619ccb5f614123fde5c4eeab805f`; API-key UI `receipt.json`, SHA-256 `ca6e9b7586d9f7c681cda8c3e39677b91e4f38348c8c4df6808a21cbb59cabe0` (overall `FAIL` only because human-session runtime-health was unavailable to that principal). The earlier `5.18.0` proof on 2026-08-27 and `5.17.1` proof on 2026-08-25 also remain historical; the latter included a live Claude Code ↔ Codex relay check.
...
| Last verified runtime | 2026-09-07: live server `26.09.07`, exact source `1652e1a3fa04207aed37eb4eea0bb236746463fa`, image digest `sha256:60264e059c66cc8abaf9eb0803635cb7f58e94ec1b3dbef5b14cbf2d4db275f4`, public health `200/ok`, deployment identity `ppm`, and matching Mac CLI/daemon `26.09.07`. Evidence: [PAI-947](https://pm.barta.cm/projects/6/issues/PAI-947) + [PAI-949](https://pm.barta.cm/projects/6/issues/PAI-949). |
...
**26.09.07 release evidence:** The protected nixcfg pin was merged through PR 540 at `289791bbe5d19e0ee4276fa926d98535fa9d7efb`. The stopped-volume archive and manifest were captured before activation. Installed CLI black-box and all 6 headless-browser fixture checks passed against static assets extracted from the actual running container; `index-CAT5wJ9i.js` SHA-256 `38d0d1b10b8fac36c9a61af467795b841406c7a7c2b35e478aa4d13a6c320051` matched the exact container asset. These checks do not close the remaining human-session, clean-OS, cross-machine, accessibility, or UI draft-refresh boundaries.
...
| **F-10** | 2026-04-26 | both | The release → verify → deploy → doc-sync gap surfaced repeatedly: README / docs / public-site copy / brand assets drift between code releases | Without an explicit reminder, the doc pass after release was easy to skip | `scripts/release-doc-sync.sh` + `just doc-sync` recipe + the four-step flow documented in DEPLOY.md | PAI-187 |
...
- **[`paimos.inspr.at/#trust`](https://paimos.inspr.at/#trust)** — the public trust posture references this register's deployment evidence.
