...
The register contains two validated deployment histories. As of 2026-06-30, ppm is the only active deploy target; the second-operator instance is historical and inactive since the operator exit in June 2026. The current ppm proof is the live `26.09.06.21.31` server deployment verified on 2026-09-06 using public `/api/health`, exact image/source identity, and schema `180`. The release passed signature, SBOM-attestation and build-provenance verification before its protected composeStack pin-bump. A full stopped-volume backup preceded activation. Local Mac CLI/daemon activation, authenticated Habitat verification, and public-site deployment remain separate pending checks. Operational evidence: PAI-928; `04-exact-live-verification.json`, SHA-256 `b0c2b36946a3260a3292103377132e31a87ce8ee80522d53a4168927983bc89e`. The earlier `5.18.0` proof on 2026-08-27 and the `5.17.1` proof on 2026-08-25 remain historical; the latter included a live Claude Code ↔ Codex relay check.
