...
> **Status:** architecture only. This document proposes follow-up work; it does
> not define a released API and does not change `VERSION`.
>
...
No ticket below is implemented by this architecture-only change.

### Slice 1 — durable delivery intent
...
### Slice 2 — target registry and delivery state
...
### Slice 4 — message-level Codex delivery
...
- No production adapter, handler, migration, or `VERSION` change in this
  architecture work.
