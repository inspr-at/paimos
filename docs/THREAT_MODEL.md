...
| **INV-EXTSTAGE-07** | Reporter registration and prerequisite setup is an authenticated internal admin plane outside the frozen adapter routes. Every mutation reauthorizes the current editor/project/delivery binding, requires canonical idempotency, writes mandatory safe audit, and accepts only exact server-owned IDs and closed fields. A sealed set contains 0–16 explicitly required/optional rows; only required rows gate owner success. Discovery returns current non-revoked safe registrations only. | `externalstage/service.go`, `handlers/external_stage_admin*.go`, literal Agent Mode routes | `externalstage/*_test.go`, `handlers/external_stage_admin*_test.go`, `cmd/paimos/cmd_external_stage_test.go` |
